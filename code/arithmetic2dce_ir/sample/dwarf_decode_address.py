...
def process_file(filename, address, Line):
...
def dfs(node, visited, parent, cfg):
    visited.append(node.address)

    for bb_dst_addr, branch_type in node.branches:
        for bb_src in cfg.basic_blocks:
            if bb_src.address == bb_dst_addr:
                neighbour = bb_src
                break
        if not bb_dst_addr in visited:
            if dfs(neighbour, visited, parent, cfg):
                return True
...
def is_node_in_cycle(start_node,cfg):
    visited = []
    return dfs(start_node, visited, start_node, cfg)
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1], 'rb') as f:
        ...
        for i in md.disasm(ops, addr):
            ...
            num += process_file(sys.argv[1], addr, int(Line))
        #res = str(Line)+': '+str(num)+'\n'
        res = str(num)
        with open(sys.argv[2],'w') as outf:   # .txt可以不自己新建,代码会自动新建
            print(res)
            outf.write(res)     # 写入
