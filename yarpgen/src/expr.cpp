...
ScalarVarUseExpr::create(std::shared_ptr<PopulateCtx> ctx) {
    ...
    avail_vars = ctx->getExtOutSymTable()->getAvailVars();
    if (avail_vars.size() == 0 ||
        (double)rand() / RAND_MAX < 0.75) { // can change prob here
        avail_vars = ctx->getExtInpSymTable()->getAvailVars();
    }
...
AssignmentExpr::create(std::shared_ptr<PopulateCtx> ctx) {
    ...
    if ((out_kind == DataKind::VAR || ctx->getLoopDepth() == 0)) {
        if (ctx->getExtOutSymTable()->vars.size() < 20) {
            ...
            if ((double)rand() / RAND_MAX < 0.5) { // can change prob here
                ctx->getExtOutSymTable()->addVar(new_var);
                ctx->getExtOutSymTable()->addVarExpr(
                    std::make_shared<ScalarVarUseExpr>(new_var));
            }
            else {
                auto extInpSymVec = ctx->getExtInpSymTable()->getVars();
                int randomi = rand() % extInpSymVec.size();
                auto extInpSymVec_iter = extInpSymVec.begin();
                for (int i = 1; i < randomi; i++) {
                    extInpSymVec_iter++;
                }
                new_var = std::shared_ptr<ScalarVar>(*extInpSymVec_iter);
            }
            auto new_scalar_use_expr =
                std::make_shared<ScalarVarUseExpr>(new_var);
